...
# Training hyperparameters (aligned with your ChanFormer)
SEQ_LEN       = 90
LABEL_LEN     = 45
PRED_LEN      = 1
EPOCHS        = 6000
...
def get_patchtst_config(n_features: int):
    return {
        ...
        'freq': 'd',
        'factor': 3,
        'patch_len': 12,    # ← typical PatchTST value; adjust if needed
        'stride': 6,
    }
...
def run_patchtst_scenario(scenario: str, file_paths: dict, assets: list):
    print("\n" + "="*80)
    print(f"RUNNING PatchTST FOR {scenario.upper()} ({len(assets)} assets)")
    print("="*80)
    ...
    print(f"→ Number of channels: {n_features} (should be 50)")
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            x = x + torch.randn_like(x) * 0.0015   # noise augmentation
        ...
        val_avg = val_loss / n_samples if n_samples > 0 else float('inf')

        scheduler_cosine.step()
        scheduler_plateau.step(val_avg)

        if (epoch + 1) % 50 == 0:
            lr = optimizer.param_groups[0]['lr']
            print(f"[{epoch+1:5d}] train: {train_avg:.7f}  val: {val_avg:.7f}  lr: {lr:.2e}")
    ...
    if best_state is not None:
        ...
        print(f"→ Loaded best weights (val loss: {best_val_loss:.8f})")
    ...
    print(f"\nFinal results for {scenario} PatchTST:")
    ...
    for i, col in enumerate(columns):
        pred_prices[:, i] = scalers[col].inverse_transform(preds_test[:, i].reshape(-1, 1)).ravel()
    ...
    df_export.to_csv(f'{scenario}_forecasts_{model_name}_full_ohlcv.csv')

    # Save metrics
    prefix = f"{scenario}_patchtst_"
    pd.DataFrame([{
        'model': model_name,
        'scenario': scenario,
    ...
    }]).to_csv(f'{prefix}global.csv', index=False)

    pd.DataFrame(per_asset_results).to_csv(f'{prefix}per_asset.csv', index=False)
    ...
    }]).to_csv(f'{prefix}overfit.csv', index=False)

    print(f"\nResults saved with prefix '{prefix}':")
    print(f"  • {prefix}global.csv")
    print(f"  • {prefix}per_asset.csv")
    print(f"  • {prefix}overfit.csv")
    print(f"  • {scenario}_forecasts_{model_name}_full_ohlcv.csv")
...
if __name__ == "__main__":
    ...
    for scen_name, paths, asset_list in scenarios:
        run_patchtst_scenario(scen_name, paths, asset_list)

    print("\nAll PatchTST benchmarks completed.")